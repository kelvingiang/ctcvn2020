...
class Admin_Metabox_Order_By {
    ...
    public function create() {
        ...
        $screen = array('silder', 'advertise', 'friend', 'supervisor', 'brach'); // CAC POST VA CUSTOMER POST CHO PHEP METABOX NAY HIEN THI
    ...
    public function display($post) {
        ...
        <div class="row-one-clo">
            <div class="cell-one"><label class="admin-title">排序 <i style="font-size: 10px">數字大先顯示</i></label> </div>
            <div class="cell-two">
                <input type="text" 
                       class="type-number"
                       id="txt_order" 
                       name="txt_order" 
                       value="<?php echo get_post_meta($post->ID, '_metabox_order_by', true) ?>" />
            </div>
        </div>
    ...
    public function save($post_id) {
        ...
        update_post_meta($post_id, '_metabox_order_by', sanitize_text_field($_POST['txt_order']));
    }
