...
// LAY GIA TRI MA THONG QUA QUA HAM get_country SHOW TEN RA
    ...
    public function column_img($item) {
        if (!empty($item['img'])) {
            echo '<img style="width:100px" src=" ' . WB_URL_IMAGES_GUESTS . $item["img"] . ' " />';
        }
    }
...
//            XOA HINH TRONG FOLDER
    ...
    public function saveItem($arrData = array(), $option = array()) {
        ...
        if (isset($arrData['hidden_barcode']) and empty($arrData['hidden_barcode'])) {
            ...
            if ($arrData['sel_country'] != $arrData['hidden_country']) {
                ...
                //$oldBarcode =    iconv('UTF-8','BIG5', WB_DIR_IMAGES_QRCODE.$arrData['hidden_fullname'].'-'.$arrData['hidden_barcode'] . '.png');
                $oldBarcode = $arrData['hidden_barcode'] . '.png';

                if (is_file(WB_DIR_IMAGES_QRCODE . $oldBarcode)) {
                    unlink(WB_DIR_IMAGES_QRCODE . $oldBarcode);
                }
            } else {
    ...
    public function createbarcode($aa) {
        ...
        file_put_contents(WB_DIR_IMAGES_QRCODE . $bar . '.png', $generatorPNG->getBarcode($bar, $generatorPNG::TYPE_CODE_128, 2, 30));
        return $bar;
    }
...
//    $name = str_replace($pattern, $replacement, $subject);
        ...
        $filepath = WB_DIR_IMAGES_QRCODE . $filename . '.png';
        // L M Q H
        ...
        // $newName =  iconv('UTF-8','BIG5',WB_DIR_IMAGES_QRCODE.$name.'-'.$filename .'.png');
