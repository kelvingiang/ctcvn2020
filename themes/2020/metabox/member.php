...
//    $m_enfullname       = get_post_meta($post->ID, 'm_lastname', true) .','.get_post_meta($post->ID, 'm_midename', true).'-'.get_post_meta($post->ID, 'm_firstname', true);
        ...
        <div class="row-all">
            ...
            <img src="<?php echo get_image('avata/' . $m_image); ?>" style="width:50px; height: 50px; border-radius: 3px; border: 1px #000 solid"/>
        </div>
        <div class="row-one-clo">
            <div class="cell-one"><label for="m_user" class="label-admin"><?php _e('帳 號', 'suite'); ?></label></div>
            <div class="cell-two"><input type="text" required  pattern=".{3,}"  placeholder="小心輸入,此欄目以後不可修改" title="minlegh 5 chars"  name="m_user" id="m_user" <?php echo (($m_user) ? 'disabled' : '' ) ?> value="<?php echo ((!isset($m_user) || $m_user != '') ? $m_user : ''); ?>" /> </div>       
        ...
        <div class="row-one-clo">
            <div class="cell-one"><label for="m_fullname" class="label-admin"><?php _e('Full Name Chinses', 'suite'); ?></label></div>
            ...
            <div class="cell-two">
                <select  id="m_Country" name="m_country" >
                    ...
                    foreach ($mylist->countryList() as $key => $val) {
                        ?>
                        <option value='<?php echo $key ?>' <?php echo $m_country == $key ? 'selected' : '' ?>  > <?php echo $val ?> </option>
                    <?php } ?>
                </select>
            </div>
        </div>
        ...
        <div class="row-one-clo">
            <div class="cell-one"> <label for="m_birthdate" class="label-admin"><?php _e('Birth Of Date', 'suite'); ?></label></div>
        ...
        <div class="row-one-clo">
            ...
            <div class="cell-two"><input type="email" style='width: 382px' required placeholder="小心輸入,此欄目以後不可修改"   name="m_email" id="m_email" <?php echo $m_email == '' ? '' : 'disabled' ?>  value="<?php echo ((!isset($m_email) || $m_email != '') ? $m_email : ''); ?>" /></div>
        ...
        <div class="row-one-clo">
            <div class="cell-one"><label for="m_tax_company" class="label-admin"><?php _e('Tax Company', 'suite'); ?></label></div>
        ...
        <div class="row-one-clo">
            <div class="cell-one"><label for="m_tax_address" class="label-admin"><?php _e('Tax Address', 'suite'); ?></label></div>
