        ...
        <?php if ($_GET['id'] == '') {
            ...
            <form id="f_article" method ="post" action="#">
                <div style="width: 98%; margin-left: 20px">
                    <!-- phan lay category  -->
                    <div class='row' >
                        <div class='col-md-3'> <label class="label-title"><?php echo _e('Category', 'suite') ?></label></div>
                        <?php
                        $argsCate = array(
                            'type' => 'forum',
                            'orderby' => 'meta_value',
                        ...
                        <div class='col-md-9' > 
                            <select id="cate" name="cate" class="selectmenu" style="width: 180px">
                                <?php foreach ($categories as $cate) { ?>
                                    <option value="<?php echo $cate->term_id ?>" 
                                    <?php
                    ...
                    <div class="row">
                        ...
                        <div class='col-md-12'> 
                            ...
                            <script>
                                ...
                                CKFinder.setupCKEditor(editor, '<?php echo get_lib_uri('class/ckfinder/') ?>');
                            </script>
                        </div>
                    </div>
            ...
            ?>
            <form id="f_update_article" method ="post" action="<?php $_SERVER["REQUEST_URI"] ?>"> <!-- giu link hien tai -->
                <div>
                    <!-- phan lay category  -->
                    <div class="content" style="display: none">
                        ...
                        <select id="update_cate" name="update_cate" class="selectmenu" style="width:150px">
                            <?php foreach ($categories2 as $cate2) { ?>
                                <option value="<?php echo $cate2->term_id ?>" 
                                <?php
                    ...
                    <div class="content">
                        ...
                        <script>
                            var editor = CKEDITOR.replace('update_editor', {customConfig: 'custom-config.js'});
                            CKFinder.setupCKEditor(editor, '<?php echo get_lib_uri('class/ckfinder/') ?>');
                        </script>
                    </div>
                ...
                while ($myQuery->have_posts()):
                    ...
                    <div class ="row-item row">
                        <span class="col-8" >
                            ...
                            <label style="font-size: 8px; color: #666666"> <?php _e('發表日期 ', 'suite'); ?> : <?php echo $post->post_date ?></label>
                        </span>
                        <span class="col-4" style=" float: right">
                            <a class="edit-item"  href="<?php echo esc_attr(add_query_arg(array('id' => $post->ID), home_url('article'))); ?>"> <?php _e('Edit', 'suite'); ?> </a> |
